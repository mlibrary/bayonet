  ...
  job.addFiles = function(files) {
    ...
    internal.existingFiles.forEach((names, path) => {
      for (const file of files[path]) {
        ...
        if (file === "Thumbs.db") {
          internal.filesToDelete.push(path + "/Thumbs.db");
          continue;
        }
        ...
        if (names.has(match[1]))
          ...
          for (const pattern of internal.imagePatterns) {
            if (pattern.volume !== path)
              continue;

            if (!file.match(pattern.image))
              continue;

            names.set(match[1], {});
            names.get(match[1]).expected
              = internal.extensions.get(pattern.template);
            names.get(match[1]).existing = new Set([match[2]]);
    ...
    internal.existingFiles.forEach((names, path) => {
      names.forEach((extensions, name) => {
        ...
        } else if (extensions.existing.size > 1
                   || !extensions.existing.has("blk")) {
...
const setUpAndValidate = internal => {
  ...
  if (!internal.lines[1].match(/^Version[ =]3\.90$/))
...
const getTemplatePair = (lines, n) => {
  ...
  return {
    ...
    "image": primeToRegex(imageMatch[2]),
...
const primeToRegex = s => {
  const pieces = s.split(".");
  pieces.pop();
  const filename = wildcardToRegex(pieces.join("."));
